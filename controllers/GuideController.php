...
class GuideController
{
    ...
    {
        if (empty($_POST['name']) || empty($_POST['email']) || empty($_POST['sdt']) || empty($_POST['exp']) || empty($_POST['language'])) {
        ...
        if ($this->modelGuide->insertGuide($name, $email, $sdt, $exp, $language)) {
    ...
    public function guideUpdate()
    {
        ...
        if ($id <= 0 || empty($name) || empty($email) || empty($sdt) || empty($exp) || empty($language)) {
        ...
        if ($this->modelGuide->updateGuide($id, $name, $email, $sdt, $exp, $language)) {
