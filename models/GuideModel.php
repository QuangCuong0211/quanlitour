...
class GuideModel
{
    ...
    // Thêm khách hàng
    public function insertGuide($name, $email, $sdt, $exp, $language)
    {
        $sql = "INSERT INTO guide (name, email, sdt, exp, language) 
                VALUES (?, ?, ?, ?, ?)";
        $stmt = $this->conn->prepare($sql);
        if (!$stmt) {
            return false;
    ...
    // Cập nhật khách hàng
    public function updateGuide($id, $name, $email, $sdt, $exp, $language)
    {
        $sql = "UPDATE guide SET name = ?, email = ?, sdt = ?, exp = ?, language = ? WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        if (!$stmt) {
            return false;
        }
