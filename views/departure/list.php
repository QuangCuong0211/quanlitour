...
<div class="content">
    <div class="card">
        ...
        <a href="?act=departure-add" class="btn">+ Thêm Lịch Khởi Hành</a>

        <br><br>

        <?php if (empty($departures)): ?>
            <div class="empty">
                <p>Chưa có lịch khởi hành nào. <a href="?act=departure-add">Thêm lịch khởi hành mới</a></p>
            </div>
        <?php else: ?>
            <table>
                <thead>
                ...
                <tbody>
                    <?php foreach ($departures as $dept): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($dept['id']); ?></td>
                        <td><?php echo htmlspecialchars($dept['tour_name'] ?? 'N/A'); ?></td>
                        <td><?php echo date('d/m/Y', strtotime($dept['departure_date'])); ?></td>
                        <td><?php echo date('d/m/Y', strtotime($dept['return_date'])); ?></td>
                        <td><?php echo htmlspecialchars($dept['guide_name'] ?? 'N/A'); ?></td>
                        <td><?php echo $dept['seats_available']; ?></td>
                        <td>
                            <span class="status <?php echo $dept['status'] == 'active' ? 'active' : 'inactive'; ?>">
                                <?php echo $dept['status'] == 'active' ? 'Hoạt động' : 'Không hoạt động'; ?>
                            </span>
                        </td>
                        <td>
                            <div class="actions">
                                <a href="?act=departure-edit&id=<?php echo $dept['id']; ?>" class="btn btn-edit">Sửa</a>
                                <a href="?act=departure-delete&id=<?php echo $dept['id']; ?>" class="btn btn-danger" onclick="return confirm('Bạn chắc chắn muốn xóa?');">Xóa</a>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
